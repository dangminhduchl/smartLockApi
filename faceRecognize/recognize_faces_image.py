...
def recognize_faces_image(encoding, image, detection_method):
    ...
    data = pickle.loads(open(encoding, "rb").read())      # loads - load từ file


    # load image và chuyển từ BGR to RGB (dlib cần để chuyển về encoding)
    _image = cv2.imread(image)

    image = cv2.resize(_image, (0, 0), fx=0.5, fy=0.5)
